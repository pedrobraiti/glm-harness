...
- **Rate limiter (`launcher/rate-limiter.mjs`):** feito sob medida pro free tier da NVIDIA. Fila com limite de concorrência (nada é descartado); ao tomar um 429, **pausa TODO o tráfego em silêncio total** (o bloqueio da NVIDIA REINICIA a cada contato) com **backoff exponencial + jitter** (base → 2x → 4x... teto 30min, contador global que só zera com sucesso real) e **retoma sozinho** — a sessão só percebe uma resposta demorada, sem "continue" manual. Inclui **trava de cota diária**: conta cada contato com a NVIDIA numa janela móvel de 24h (persistida em `logs/rpd-ledger.json`) e segura novas chamadas antes do teto empírico de ~1000/dia que bloquearia a conta inteira; o consumo aparece na statusline a partir de 50%. Config em `limiter-config.json` (**hot-reload** — editar já vale): `maxConcurrent` (default 2; recomendado 1), `cooldownSeconds` (default 75), `maxAttempts` (12), `dailyBudget` (950). Dentro do `glm`, o comando **`/requisitions`** mostra/ajusta (`/requisitions 1`, `/requisitions 1 90 950`). Estado vivo: `http://127.0.0.1:3457/glm-limiter/health`; logs em `logs/limiter.log`.
...
- **O que esperar do free tier na prática** (medido, não teoria): o 429 da NVIDIA vem **sem nenhum header** de limite (nem `retry-after`), o castigo **reinicia a cada contato** durante o bloqueio, há um teto diário não-publicado de ~1.000 requisições/24h (por conta, todos os modelos), e o endpoint gratuito **corta o contexto em ~202k tokens** (falha com HTTP 500 acima — por isso o harness compacta em 180k, apesar de o modelo suportar 1M). O padrão de uso pesado é: funciona um bom tempo → seca → o limiter pausa em silêncio (statusline mostra `⏸ 429 · retoma em Xs` ou `⏸ cota diária`) → volta sozinho. **Regra de ouro: quando demorar, não cancele nem reenvie** — cada toque durante o bloqueio o estende. Uso leve/moderado: você provavelmente nem nota. Motor principal o dia todo: considere o endpoint pago.
